...
/obj/machinery/hydroponics/examine_more(user)
	if(myseed && (in_range(user, src) || isobserver(user)))
		. += "<span class=info>You examine the plant to get a better view of its harvest...</span>"
		switch(src.myseed.potency)	// Check potency
			if(0 to 20)
				. += "<span class='warning'>The harvest's size is tiny, the fruits are small and insignificant.</span>"
			if(21 to 40)
		...
		switch(src.myseed.weed_rate)		//Checks Weed Growth Rate
			if(0 to 2)
				. += "<span class='nicegreen'>The weed growth around the plant appears to be of a miniscule amount...</span>"
			if(3 to 5)
